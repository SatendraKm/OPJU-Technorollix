...
import { sendResetPasswordEmail } from "@/actions/mailer";

const otpSchema = z.object({
  otp: z.string().min(6, { message: "OTP must be 6 digits." }),
});

export default function VerifyOtpReset({ onNext }: { onNext: () => void }) {
  ...
  const handleResendOtp = async () => {
    setIsResending(true);
    const recipientEmail = sessionStorage.getItem("resetPasswordEmail");
    const newOtp = Math.floor(100000 + Math.random() * 900000).toString();
    sessionStorage.setItem("resetPasswordOtp", newOtp);

    try {
      if (recipientEmail) {
        // Optionally, call sendResetPasswordEmail to resend the OTP
        await sendResetPasswordEmail(recipientEmail, newOtp);
        toast({
          title: "OTP Resent",
